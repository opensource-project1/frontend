        ...
        <div className="screen-box">
          ...
          <div className="tab-content active" style={{ display: 'flex', flexDirection: 'row', gap: '200px' }}>
            <div className="left-panel" style={{ flex: 1, minWidth: 0 }}>
              <div className="pill-heading">오늘 할 일</div>
              <input
                type="text"
                value={todoInput}
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                {todos.map((todo, idx) => (
                  <li key={idx} style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <button onClick={() => handleTodoDelete(idx)} style={{ marginLeft: 8, background: 'none', border: 'none', color: '#ffb0d2', fontWeight: 'bold', fontSize: 18, cursor: 'pointer' }} title="삭제">×</button>
                  </li>
                ))}
              </ul>
            </div>
            <div className="right-panel" style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start' }}>
              <div className="pill-heading">집중 타이머</div>
              <div className="timer-display" style={{ marginBottom: 32, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div className="time" style={{ fontSize: 28, fontWeight: 'bold', marginBottom: 16 }}>{formatTime(timeLeft)}</div>
